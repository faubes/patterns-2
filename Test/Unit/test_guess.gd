...
func test_guess_request_cell():
	var design = _test_design()
	var guess = Guess.new(design)
	assert_eq(guess.probe(Vector2(0,0)), "A", "Can sample symbols from design")
...
func test_guess_incomplete():
	...
	assert_false(guess.guessed(Vector2(0,0)), "Value of guess for cell (0,0) should be null")
	assert_true(guess.guessed(Vector2(0,1)), "Value of guess for cell (0,1) should be B")
	assert_false(guess.validate(), "Partial guess is not valid")
...
func test_guess_complete():
	...
	assert_eq(_complete_guess.score_all(), 36, "Perfect guess scores 36")
